...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        ent_weight = float(kwargs.get("ent_weight", 1.0))
        if ent_weight > 0:

            def _neighbor_entropy_avg_from_hidden(self, hidden_states_tuple):
                entropy_layers = [26, 27, 28, 29, 30]

                B, T_total, _ = hidden_states_tuple[0].shape
                device = hidden_states_tuple[0].device
                num_layers = float(len(entropy_layers))
                tail_len = 128
                start = max(0, T_total - tail_len)
                T = T_total - start
                if T == 1:
                    return torch.zeros((B, T), device=device)

                S_layers = torch.zeros((B, T), device=device)

                for li in entropy_layers:
                    h = hidden_states_tuple[li][:, start:, :]
                    logits_l = self.lm_head(h).float()
                    p = F.softmax(logits_l, dim=-1)

                    H_ent = -(p * (p.clamp(min=1e-12).log())).sum(dim=-1)  # (B, T_last)
                    H_max = math.log(p.shape[-1]) if p.shape[-1] > 0 else 1.0
                    H_norm = (H_ent / max(H_max, 1e-12)).clamp(0.0, 1.0)  # (B, T_last)
                    S_layers += H_norm
                if T == 2:
                    idx1 = torch.tensor([1, 0], device=device, dtype=torch.long)
                    idx2 = torch.tensor([1, 0], device=device, dtype=torch.long)
                else:
                    ...
                    idx1[0], idx2[0] = 1, 2
                    idx1[1], idx2[1] = 0, 2
                    idx1[T - 2], idx2[T - 2] = T - 3, T - 1
                    idx1[T - 1], idx2[T - 1] = T - 2, T - 3

                # neighbor1 = S_layers[:, idx1]
                # neighbor2 = S_layers[:, idx2]
                neighbor1 = S_layers
                neighbor2 = S_layers
                selfval = S_layers

                context_sum = neighbor1 + neighbor2 + selfval  # (B, T_last), ∈ [0, 3*num_layers]
                denom = 3.0 * num_layers
                confusion_score = (context_sum / denom).clamp(0.0, 1.0)
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    if cfg_scale > 0.:
                        ...
                        # Forward pass
                        outputs = self.model(inputs_embeds=x_embeds, output_hidden_states=(ent_weight > 0))
                        hidden_states_cond = outputs.hidden_states if (ent_weight > 0) else None
                        logits = self.lm_head(outputs[0]).float()

                    for token_id in [126081, 126080, 126346, 126347]:
                        logits = logits.to(mask_index.device)
                    ...
                    if ent_weight > 0:
                        ...
                        neighbor_entropy_avg = _neighbor_entropy_avg_from_hidden(hidden_states_cond)  # (1, T)
                        selection_score =  confidence - 0.25 * neighbor_entropy_avg
                        # selection_score = confidence / neighbor_entropy_avg
                    else:
                        selection_score = confidence
                    transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                    for j in range(selection_score.shape[0]):
                        k = int(num_transfer_tokens[j, i])
                        if k > 0:
                            if torch.isfinite(selection_score[j]).any():
                                _, select_index = torch.topk(selection_score[j], k=k)
                                transfer_index[j, select_index] = True
        ...
        if conversation_ids is not None:
            ...
            if attention_mask is not None:
                ...
                attention_mask = attention_mask.expand_as(
                    conversation_mask)  # (batch, 1, 1, length) -> (batch, 1, length, length)
